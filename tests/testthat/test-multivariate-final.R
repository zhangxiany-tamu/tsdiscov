# Final comprehensive test for 61 multivariate features (after Phase 1-3)

test_that("All multivariate features work correctly - 61 total", {
  set.seed(42)
  ...
  # Should have exactly 61 features
...
test_that("Individual feature sets have correct counts", {
  ...
  # Covariance: 5 (removed cov_nuclear_norm and cov_determinant, kept log_determinant)
  cov_feats <- suppressWarnings(ts_features_multivariate(X, features = "covariance"))
  expect_equal(length(cov_feats), 5)
  ...
  # Total Correlation: 2
  tc_feats <- ts_features_multivariate(X, features = "total_correlation")
  expect_equal(length(tc_feats), 2)

  # Lag Structure: 4
  lag_feats <- ts_features_multivariate(X, features = "lag_structure")
  expect_equal(length(lag_feats), 4)

  # Network: 5
  net_feats <- ts_features_multivariate(X, features = "network")
  expect_equal(length(net_feats), 5)

  # Total: 15 + 15 + 5 + 8 + 7 + 2 + 4 + 5 = 61
  expect_equal(15 + 15 + 5 + 8 + 7 + 2 + 4 + 5, 61)
})
...
test_that("ts_features_all works with all input types", {
  ...
  expect_equal(length(features_mat), 61)  # Multivariate features

  # List input
  X_list <- list(a = rnorm(150), b = rnorm(150), c = rnorm(150))
  features_list <- ts_features_all(X_list)
  expect_equal(length(features_list), 61)
})
...
test_that("Features work on different data types", {
  ...
  expect_equal(length(feat_indep), 61)
  ...
  expect_equal(length(feat_corr), 61)
